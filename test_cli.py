...
class Test(unittest.TestCase):
    ...
    def test_06_find(self):
        ...
        self.assertTrue("iuids" in result)
        self.assertEqual(result["iuids"], ["docid3"])
        self.assertFalse("docs" in result)
        p = self.execute("find", "ix", "id3", "--docs")
        self.assertEqual(p.returncode, 0)
        result = json.loads(p.stdout)
        self.assertTrue("count" in result)
        self.assertEqual(result["count"], 1)
        self.assertFalse("iuids" in result)
    ...
    def test_07_range(self):
        ...
        self.assertTrue("iuids" in result)
        self.assertEqual(set(result["iuids"]), set(["docid3", "docid4"]))
        self.assertFalse("docs" in result)
        p = self.execute("range", "ix", "id3", "id4", "--docs")
        self.assertEqual(p.returncode, 0)
        result = json.loads(p.stdout)
        self.assertTrue("count" in result)
        self.assertEqual(result["count"], 2)
        self.assertFalse("iuids" in result)
        self.assertTrue("docs" in result)
        self.assertTrue(len(result["docs"]), 2)
